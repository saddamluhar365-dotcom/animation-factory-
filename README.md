...
The Gemini adapter uses the current `x-goog-api-key` authentication header and defaults to `gemini-3.8-flash`. The model can be overridden without changing code by setting `YT_AUTO_GEMINI_MODEL` in the process environment.

If a provider test fails, the API window now displays the provider's HTTP status and safe error message instead of only showing `FAILED`. API keys themselves are never written to logs.
...
- PostgreSQL is used for durable engine memory when the configured database is available. The engine also supports SQLite-backed test instances for deterministic CI coverage.
