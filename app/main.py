...
class App(tk.Tk):
    def __init__(self):
        ...
        self._build(); self.after(200,self.first_run)
    ...
    def write_log(self,msg):
        self.status.set(msg); self.log.configure(state="normal"); self.log.insert("end",msg+"\n"); self.log.see("end"); self.log.configure(state="disabled")
    ...
    def api_window(self,first=False):
        win=tk.Toplevel(self); win.title("API Configuration"); win.geometry("720x520"); win.transient(self); win.grab_set()
        ttk.Label(win,text="API Configuration",font=("Segoe UI",16,"bold")).pack(pady=12)
        ttk.Label(win,text="Paste a key, press TEST & ADD. Successful live APIs are registered automatically.").pack(pady=4)
        ...
        for provider,label in PROVIDERS:
            ...
            e=ttk.Entry(box,show="*",width=65); e.pack(side="left",fill="x",expand=True)
            result=ttk.Label(box,text="") ; result.pack(side="left",padx=8)
            entries[label]=(e,result)
            ttk.Button(box,text="TEST & ADD",command=lambda p=label,e=e,r=result:self.test_api(p,e,r)).pack(side="right")
        ttk.Label(win,text="Settings can store multiple keys per provider; only one key per required provider is needed initially.").pack(pady=5)
    ...
    def test_api(self,provider,e,result):
        ...
        def work():
            ...
            self.after(0,lambda:(result.config(text="✓ LIVE" if ok else "✗ FAILED"),self.write_log(f"{provider}: {'LIVE' if ok else 'FAILED'}")))
    ...
    def close_setup(self,win,first):
        ...
        if first:
            ...
            from .storage import save_config; save_config(cfg)
    ...
    def reference_setup(self):
        ...
        save_reference_profile(results); self.write_log("Reference profile saved.")
